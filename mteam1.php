					...
					<div class="card-body">
						<table class="table table-condensed table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$types = $conn->query("SELECT *,concat(firstname) as name,concat(hod_name) as name1,concat(m_team) as name2, concat(AdvancedAmount) as caddress , concat(Context) as caddress1 FROM persons where status=0 AND m_team='M Team 1' order by name asc");
