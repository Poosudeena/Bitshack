...
Class Action {
	...
	function save_person(){
		...
		$data .= ", AdvancedAmount = '$AdvancedAmount' ";
	...
	function get_pdetails(){
		extract($_POST);
		$get = $this->db->query("SELECT *,concat(firstname) as name,  concat(hod_name) as name1,concat(m_team) as name2,concat(AdvancedAmount) as caddress, concat(Context) as caddress1,  concat(RequestedSpecialLab) as caddress3 FROM persons where tracking_id = $tracking_id ");
		$data = array();
		if($get->num_rows > 0){
