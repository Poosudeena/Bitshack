...
$ids = implode(",",$ids);
$qry = $conn->query("SELECT *,concat(firstname) as name,concat(lastname) as name1, concat(hod_name) as name2,concat(m_team) as name3,concat(RequiredComponents) as caddress,concat(QuantitiesRequired) as caddress1,concat(Text) as caddress2, concat(Dept) as caddress3,concat(RequestedSpecialLab) as caddress4 FROM persons where id in ($ids) ");
